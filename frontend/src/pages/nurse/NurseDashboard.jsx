  ...
  return (
    <div className="max-w-4xl mx-auto pt-24 px-4">
      <h2 className="text-3xl font-bold mb-8 text-blue-700 flex items-center gap-3">
        <span role="img" aria-label="nurse" className="text-blue-400 text-4xl">
          🩺
        </span>
      ...
      <div className="mb-10 bg-white rounded-2xl shadow-lg p-8 border border-blue-100">
        <h3 className="text-xl font-bold text-blue-700 mb-6 flex items-center gap-2">
          <span role="img" aria-label="test" className="text-blue-400 text-2xl">
            🧪
          </span>
        ...
        {assignedTests.length === 0 ? (
          <div className="text-gray-500 italic text-center py-12">
            ...
            Không có xét nghiệm nào được giao.
            <br />
            <span className="text-xs text-gray-400">
              Mã điều dưỡng: {user?.id || "không rõ"}
            </span>
            <br />
            <span className="text-xs text-gray-400">
              Nếu bạn mong đợi có xét nghiệm, hãy kiểm tra backend với
              TestAssignment có nurseId trùng và trạng thái 'assigned'.
            </span>
          </div>
        ) : (
          <ul className="space-y-6">
            {assignedTests.map((test) => (
              <li
                key={test._id}
                className="bg-blue-50 border border-blue-200 rounded-2xl px-6 py-5 flex flex-col gap-3 shadow-sm"
              >
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 justify-between">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-gray-700">
                    <span>
                      Bệnh nhân: <b>{test.patientId?.userName}</b>
                    </span>
                    <span>
                      Bác sĩ: <b>{test.doctorId?.userName}</b>
                    </span>
                  </div>
                </div>
                <div className="flex flex-col gap-3 mt-2">
                  {/* CT scan, X-quang, Siêu âm bụng: tải ảnh và ghi chú */}
                  {(test.testId?.name === "Chụp CT" ||
                    test.testId?.name === "X-ray" ||
                    test.testId?.name === "X-quang" ||
                    test.testId?.name === "Siêu âm bụng") && (
                    <div className="flex flex-col gap-2">
                      <label className="font-semibold text-xs text-blue-700">
                        Tải ảnh lên
                      </label>
                      <input
                        type="file"
                        ...
                        onClick={async () => {
                          ...
                          try {
                            await axiosInstance.put(
                              `/test-assignment/result/${test._id}`,
                              formData,
                              {
                                headers: {
                  ...
                  {(test.testId?.name === "Blood Test" ||
                    test.testId?.name === "Xét nghiệm máu") && (
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (
                              ...
                              toast.error(
                                "Vui lòng nhập kết quả xét nghiệm máu"
                              );
                              return;
                            }
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (!input?.ph && !input?.protein) {
                              toast.error(
                                "Vui lòng nhập kết quả xét nghiệm nước tiểu"
                              );
                              return;
                            }
                            await handleSubmitResult(test._id, input);
