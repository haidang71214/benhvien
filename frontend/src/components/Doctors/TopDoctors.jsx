    ...
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      ...
      <div className="w-full grid grid-cols-auto gap-4 gap-y-6 pt-5 px-3 sm:px-0">
        {doctors.slice(0, 10).map((item, index) => {
          ...
          const speciality = Array.isArray(item.specialty)
            ? item.specialty.join(", ")
      ...
      <button onClick={() => {navigate('/doctors'); scrollTo(0, 0)}} className="bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10 hover:scale-105 transition-all duration-300">
        More
      </button>
    </div>
  );
